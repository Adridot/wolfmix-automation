...
| COV-38 | 2026-09-01 | Which **writer** made the save COV-37 caught, planned and published before the measurement. The version counter increments the same for the panel and for WTOOLS, and nothing this repository reads carries an author; COV-02 found one split on a save that *changed* something, and nobody has asked what each writer touches on a save that changes **nothing**. Two no-op saves, one per author, and a diff — the set of record types each touches is the fingerprint and the deliverable. Sheet: [`corpus/experiments/COV-38/prediction.md`](../corpus/experiments/COV-38/prediction.md). The baseline is **re-read rather than assumed**: `projectChanged` was false at the last look, but the unit has since been reloaded and crashed (COV-39). Two rivals are pre-registered and both are real — neither save changes a single record, which would mean a no-op save is inert on this firmware and the regression needed an edit; or the two are indistinguishable, and version `…327` cannot be attributed by this route at all. | planned | §10 |
