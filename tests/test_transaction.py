...
def fake_link(projects, downloads):
...
class ArchiveIsATransaction(unittest.TestCase):
    ...
    def test_a_project_is_never_archived_without_its_manifest(self):
    ...
    def test_a_manifestless_archive_is_repaired_without_downloading_again(self):
        with fake_link(self.projects, self._download), \
                tempfile.TemporaryDirectory() as directory:
    ...
    def test_an_archive_that_no_longer_matches_its_manifest_stops_the_run(self):
        with fake_link(self.projects, self._download), \
                tempfile.TemporaryDirectory() as directory:
            tx.archive_projects(None, directory)
    ...
    def test_a_new_version_is_archived_beside_the_old_one(self):
        with fake_link(self.projects, self._download), \
                tempfile.TemporaryDirectory() as directory:
            tx.archive_projects(None, directory)
...
class FailedRollback(unittest.TestCase):
    ...
    def test_the_failure_is_written_and_names_the_way_out(self):
    ...
    def test_the_next_deploy_refuses_and_names_the_archive_to_restore(self):
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_a_clean_state_does_not_refuse_for_that_reason(self):
        with tempfile.TemporaryDirectory() as directory:
            self._armed_state(directory)
            args = argparse.Namespace(state_dir=directory, label="boundary",
                                      namespace="exp", port=None, timeout=1.0)
