...
| Publishable | Files |
|---|---|
| yes | `corpus/projects/00000000-0000-0000-cc21-*.wpj` (`rig-a`), `…cd21-*.wpj` (`rig-b`) — two minimal two-fixture projects |
| yes | everything under `corpus/experiments/` — generated by our experiments, no third-party content |
| **no** | `rig-c*.wpj`, `f2737ec3*.wpj` — the operator's real production show, including its patch |
| **no** | all variant-B files and `.wm`/`.wmx` sidecars — WTOOLS-side data not reviewed for third-party content |

Together the publishable set is 14 variant-A files, enough for every self-check
in `tools/` to run for someone who cloned this repository.
