...
| The two FX pages per engine ("Double Trouble") | 🟢 one `f16` slice per engine covers **both** pages; the record files each engine as a quadruple — pair, **page selector**, active — so `f7` selects the Colour page and `f23` the Move page, per group, `0` = `FX1` / `1` = `FX2`. **Validated** by two single-variable shots; `f3` is the Beam slot by structure and is **not** measured |
...
| 🔜 | `f3`/`f7`/`f23`/`f27` of record 165 — four per-group arrays the changelog does not name |
