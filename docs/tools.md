...
- **Run with no arguments = self-check.** Each tool proves its own invariant
  over the frozen corpus and exits non-zero if it cannot. `make check` runs the
  five that matter.
...
without an edit produces a byte-identical file. Self-check: the whole variant-A
corpus (25 files at last run).
