...
| `f5`, `f6` | the fixture's **travel limits**, per fixture and per axis — the `MIN`/`MAX` of the `FIXTURE LIMITS` screen, as `v/255` (COV-24). **Invert pan swaps the two.** **Absent means 0, not "unset" (COV-35):** a fixture whose four limit fields are all absent reads `0 %` four times on that screen and is **pinned** — `min == max == 0` collapses the range, and it emits 0 on both axes whatever position is recalled. A reader must not fill an absent limit with the full range. A fresh patch writes `0…255` (PATCH-01). |

**On the two wheel roles, 11 and 12, `f5`/`f6` are not limits — [correlated]
(COV-65).** They are the wheel channel's slice of record 111: `f5` its global
`110.f3` offset, `f6` its `110.f2` range count, 1224 of 1224 entries on every
device-written file. The codec keeps the axis names; a writer that moves a
profile in record 110 must recompute them, and `wpj_identities.py` reports a
file where they drift as an anomaly.
...
- `110.f1` is `pair_member` — **[correlated] (COV-67)**: present, at 1, on
  the coarse and fine channels of a 16-bit pair and on no other channel, and
  record 120 writes its pairing (`f5`/`f6`) for those channels only. It was
  declared inert; its *presence* is the reading, and the two-channel profiles
  whose second channel points at the first with no feature are the case that
  needs it.
...
### `116.f12` = the profile's channel names — **[device-confirmed]**
...
**Confirmed by the cleanest possible file (COV-43).** A project the panel
created from scratch carries a single 16-channel profile with **16** `f12`
entries aligned to its **own** channels — `Pan `, `Tilt `, `Pan/Tilt Speed`,
`Strobe`, and `Color Effect` / `Reset` on the two `Generic`, name for name what
COV-25 read off the screen and index for index the six COV-30 reconstructed
out of three other profiles' items. One profile makes mis-slicing impossible,
which is what makes it the confirming case. The field is named
`channel_names`.

**The warning stands and is now proven, not inferred.** In the eight files
WTOOLS 2.0.2 wrote, those sixteen names are spread across three other profiles
and the one they belong to carries none. A reader must not trust the alignment
blindly; a writer must not reproduce the cut.
...
|---|---|---|
| `151.f1` | — | **index into record 115**, absent = 0 — *not* a rank in display order, settled by a reorder that moved the head to the end of the list and left the index alone (COV-47). The firmware also **remaps it** when the patch shrinks: deleting the fixture below it rewrote 1 to 0 (COV-46). Both times the offset kept addressing the head it was set on, confirmed at the panel. **And it must name a fixture of the group that owns the slice** — 353 corpus entries do, 11 do not, and those eleven are one damaged project (COV-48) |
...
ranges in the installed App image and independently disassembled, and
UPLOAD-08 wrote the device with them on 2.0.19: 179 chunks acknowledged at a
158 ms median — the vendor's own cadence — the counts unchanged, the icons on
screen after a `RESTART`. `wolfmix.py gobo-upload` exposes the event only
after verifying the vendor bundle manifest, backup, patched-image manifest,
operator-reviewed sheet and a byte diff confined to the declared gobo icon
windows.
...
**group** — with no donor carrying the patch. The profile arrives as a
*package* read off a project the device wrote: its 116 entry, 110/111 slices,
106 role pattern, 105 block layout and 120 template. Everything else comes
from a **skeleton** the operator owns, ideally a project the W1 created from
nothing. Rebuilt from their own packages, 83 of 111 corpus files come back
byte-identical on all nine records; the 28 others each carry a named anomaly.

| Record | Derived how |
|---|---|
| 116, 110, 111 | packages concatenated, offsets recomputed |
| 105 | one entry per block, in **display order**, pointing into 106 |
...
**The binary route is closed, not untried (BIN-01).** WTOOLS is a Flutter
application whose protobuf is the USB protocol and whose project code moves
the file as a blob; the serialiser is in the firmware, which the bundle ships
encrypted end to end. The file and the device are the oracles.
...
| Q8 | **Record 161** — three items, each a **packed array of exactly 188 varints** (COV-06). Not volatile, as this document long said: of the 564 values, exactly **6 ever differ**, and they are the same six in two unrelated projects (COV-21) — the **last value of each item**, which move together and to the same number (240 or 176), and the **first three of item 1**, which read as a little-endian **20-bit mask**, `0x0FFFFF` or `0` — and COV-45 has since read that mask: it is over the **fixtures**, those files carry twenty of them, and the highest set bit never addresses a fixture record 115 does not have (288 items, an identity). What it *selects* is still unread. The other 182 per item have not moved in 82 files, 13 projects, two authors. Its `f1` is 21, 15, 19 by item position; the record's own `f1` is 9 against 3 items, so it is not a count. FX6-05 removed the beam-sequencer hypothesis, the only one it ever had. | An attribution for any of the six. Three observed transitions of the tail (F30-04, COV-22's pair, and COV-32's `176 → 240` on a save the operator could not attribute) and one of the mask (FX6-02) are not an attribution, and a 20-bit mask has too many candidates here — a palette page, a preset page and a Live Edit page are all 20. It has a **schema** since 2026-09-01 and that changes nothing here: the schema names `items` and types the three arrays as packed, so a diff points at which of the 564 values moved instead of at 190 bytes of hex. The values themselves keep neutral keys and count `partial`, not `read`. |
