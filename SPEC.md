...
| Q8 | **Record 161** — three items, each holding a **packed array of exactly 188 varints** (COV-06, on all 228 corpus occurrences; the byte length varies only because some values pass 127, the same trap `155.f4` sprang). Its `f1` is 21, 15, 19 by item position, constant corpus-wide; the record's own `f1` is 9 against 3 items, so it is not a count. The array is nearly all zeros — two or three non-zero at the head, one at index 187 reading 160, 176 or 240. FX6-05 removed the reason to look here for the beam sequencer, which was the only hypothesis it ever had. | A meaning for any of it. The shape is no longer the obstacle; nothing says what 188 counts. It round-trips verbatim in the meantime. |
